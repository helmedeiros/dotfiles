...
@test "status_exists should return true if status type exists" {
  # Create a status file with type "test"
  status_update "test" "Test status message"
...
@test "status_get_message should return the correct status message" {
  # Create a status file with message "Test status message"
  status_update "test" "Test status message"
...
@test "status_get_prompt should return the correct prompt format" {
  # Test for dotfiles update
  status_update "dotfiles" "Dotfiles updates available"
  run status_get_prompt
  [[ "$output" == *"[DOTFILES UPDATE]"* ]]
  
  # Test for brew update
  status_update "brew" "Brew updates available"
  run status_get_prompt
  [[ "$output" == *"[BREW UPDATE]"* ]]
  ...
  # Test for unknown update type
  status_update "unknown" "Unknown updates available"
  run status_get_prompt
...
@test "status_last_check should return 0 if already checked today" {
  # Create a last check file with today's date
  date +%Y%m%d > "${DOTFILES_LAST_CHECK_FILE}"
...
@test "status_last_check should return 1 if not checked today" {
  # Create a last check file with yesterday's date
  date -j -v-1d +%Y%m%d > "${DOTFILES_LAST_CHECK_FILE}"
